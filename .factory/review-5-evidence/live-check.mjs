...
const evidenceDir = '/work/repo/.factory/review-5-evidence';
const results = [];
const requests = [];
const consoleErrors = [];
const pageErrors = [];
...
try {
  ...
  await check('clipboard denial exposes selectable fixed board and result links', async () => {
    await desktop.goto(`${base}/play?seed=REVIEW5-COPY`, { waitUntil: 'domcontentloaded' });
    ...
    assert.equal(new URL(boardLink).searchParams.get('seed'), 'REVIEW5-COPY');
  ...
  await check('independent clients open the same fixed board and keep the room on reload', async () => {
    ...
    observe(first);
    observe(second);
    await first.goto(`${base}/play?seed=REVIEW5-ROOM`, { waitUntil: 'domcontentloaded' });
  ...
  await check('five-minute boundary reaches actual loss screen', async () => {
    await desktop.goto(`${base}/play?seed=REVIEW5-LOSS`, { waitUntil: 'domcontentloaded' });
    await desktop.getByRole('button', { name: 'Start five-minute board' }).click();
    await desktop.getByRole('button', { name: 'Pause' }).click();
    await desktop.evaluate(() => {
      const key = 'daily:session:REVIEW5-LOSS';
  ...
  await check('invalid result and invalid saved session recover safely', async () => {
    ...
    await desktop.goto(`${base}/play?seed=REVIEW5-INVALID`, { waitUntil: 'domcontentloaded' });
    await desktop.evaluate(() => localStorage.setItem('daily:session:REVIEW5-INVALID', JSON.stringify({ rotations: Array(36).fill(null), elapsed: -1, turns: -9, status: 'playing', assist: false })));
    await desktop.reload({ waitUntil: 'domcontentloaded' });
    assert.equal((await desktop.locator('[data-timer]').textContent()).trim(), '5:00');
    assert.equal((await desktop.locator('[data-turns]').textContent()).trim(), '0');
    assert.equal(await desktop.evaluate(() => localStorage.getItem('daily:session:REVIEW5-INVALID')), null);
  ...
  await check('routes have correct status, titles, structure, metadata, legal content, and designed 404', async () => {
    const routes = [
      ...
      ['/missing-review-5', 404, 'Page not found — Seed Sprint']
    ];
    for (const [route, status, title] of routes) {
      ...
      await axe(desktop, route === '/missing-review-5' ? 'designed 404' : route);
    }
  ...
  await check('live requests stay same-origin with no unexpected console or page errors', async () => {
    ...
    const expected404 = consoleErrors.filter(item => item.url.endsWith('/missing-review-5') && item.text.includes('404'));
