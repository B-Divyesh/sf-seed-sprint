...
const evidenceDir = '/work/repo/.factory/review-4-evidence';
const results = [];
const requests = [];
const consoleErrors = [];
const pageErrors = [];
...
try {
  ...
  await check('clipboard denial exposes selectable fixed board and result links', async () => {
    await desktop.goto(`${base}/play?seed=REVIEW4-COPY`, { waitUntil: 'domcontentloaded' });
    ...
    assert.equal(new URL(boardLink).searchParams.get('seed'), 'REVIEW4-COPY');
  ...
  await check('independent clients open the same fixed board and keep the room on reload', async () => {
    ...
    observe(first);
    observe(second);
    await first.goto(`${base}/play?seed=REVIEW4-ROOM`, { waitUntil: 'domcontentloaded' });
  ...
  await check('five-minute boundary reaches actual loss screen', async () => {
    await desktop.goto(`${base}/play?seed=REVIEW4-LOSS`, { waitUntil: 'domcontentloaded' });
    await desktop.getByRole('button', { name: 'Start five-minute board' }).click();
    await desktop.getByRole('button', { name: 'Pause' }).click();
    await desktop.evaluate(() => {
      const key = 'daily:session:REVIEW4-LOSS';
  ...
  await check('invalid result and invalid saved session recover safely', async () => {
    ...
    await desktop.goto(`${base}/play?seed=REVIEW4-INVALID`, { waitUntil: 'domcontentloaded' });
    await desktop.evaluate(() => localStorage.setItem('daily:session:REVIEW4-INVALID', JSON.stringify({ rotations: Array(36).fill(null), elapsed: -1, turns: -9, status: 'playing', assist: false })));
    await desktop.reload({ waitUntil: 'domcontentloaded' });
    assert.equal((await desktop.locator('[data-timer]').textContent()).trim(), '5:00');
    assert.equal((await desktop.locator('[data-turns]').textContent()).trim(), '0');
    assert.equal(await desktop.evaluate(() => localStorage.getItem('daily:session:REVIEW4-INVALID')), null);
  ...
  await check('live 14-result history retains newest, removes oldest, supports view, replay, clear', async () => {
    ...
    observe(page);
    const seeds = Array.from({ length: 15 }, (_, index) => `LIVE4-${String(index + 1).padStart(2, '0')}`);
  ...
  await check('routes have correct status, titles, structure, metadata, legal content, and designed 404', async () => {
    const routes = [
      ...
      ['/result?seed=ROUTE-4&status=won&time=94&turns=32', 200, 'Shared result — Seed Sprint'],
      ['/missing-review-4', 404, 'Page not found — Seed Sprint']
    ];
    for (const [route, status, title] of routes) {
      ...
      await axe(desktop, route === '/missing-review-4' ? 'designed 404' : route);
    }
  ...
  await check('live requests stay same-origin with no unexpected console or page errors', async () => {
    ...
    const expected404 = consoleErrors.filter(item => item.url.endsWith('/missing-review-4') && item.text.includes('404'));
